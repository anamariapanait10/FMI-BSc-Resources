...
from employees
where lower(last_name) like '%e'; --5 rez
...
select next_day('03/MAR/2022', 'Monday') --conversie implicita de la string la date
from dual;  --07-MAR-22
...
from employees
order by 1;
100	12677.551099537037037037037037037037037
101	11850.551099537037037037037037037037037
102	10640.551099537037037037037037037037037
...
Select to_date('31-DEC-2022') - trunc(sysdate), to_char(sysdate, 'dd-mm-yyyy HH24:MI')
from dual; 
...
from dual; --303	03-03-2022 19:44
...
select 365 - to_char(sysdate, 'DDD') "zile ramase" , to_char(sysdate, 'DDD') "ziua din an"--31+28+2
from dual; -- 304
--31(ian)+28(feb)+2(mar) = 61
--365-61
...
---lab 4 9/10 - 03.2022  -sapt 4 - L2-pdf
[Funcþii diverse]
...
15. S? se listeze numele, salariul ,
  commission_pct si valoare calculata a comisionului pt
...
select decode('b', 'a', 'bbbbbb', 'c', 'ddd', 'bjhafkas') --null (lipseste else)
from dual;
